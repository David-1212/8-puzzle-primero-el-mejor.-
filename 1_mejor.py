...
def best_first_search(inicial, meta):
    visitados = set()   #Conjunto de estados visitados para no visitar el mismo estado más de una vez.
    frontera = PriorityQueue()  #Cola de nodos aún por explorar. Se agrega el nodo inicial.  
    node = Nodo(inicial, None, None, 0, calcular_heurisitica(inicial, meta))
    frontera.put(((9 - node.piezas_correctas), node))
    visitados.add(node.estado)
    costo_camino = 0
    
    while frontera:                         #Mientras haya nodos por explorar:
        nodo = frontera.get()[1]           #Se toma el primer nodo de la cola.
        ...
        if nodo.estado == meta:                         #Si es una meta, 
            ...
            for child in nodo.encontrar_sucesores(meta):
                if child.estado not in visitados:
                    visitados.add(child.estado)
                    frontera.put(((9 - child.piezas_correctas), child))

if __name__ == '__main__':
    estado_inicial = (1, 5, 2, 4, 0, 6, 7, 3, 8)
    ...
    nodos_camino, costo_camino = best_first_search(estado_inicial, estado_final)
